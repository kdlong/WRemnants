...
def read_matched_scetlib_dyturbo_hist(scetlib_resum, scetlib_fo_sing, dyturbo_fo, axes=None, charge=None, fix_nons_bin0=True, coeff=None):
    ...
    vars_unique = vars_dict.keys()

    vars_out = list(vars_unique)

    additional_var_hists = {}

    s = hist.tag.Slicer()

    htotal_nominal =  htotal[{"vars" : "pdf0"}]

    resum_scales = ["muB", "nuB", "muS", "nuS"]
    resum_scale_vars = [var for var in vars_unique if any(resum_scale in var for resum_scale in resum_scales)]

    if "kappaFO0.5-kappaf2." in vars_unique:
        # envelope of renormalization scale variations
        logger.info("adding envelopes including FO scale variation")

        vars_out.append("renorm_scale_min")
        vars_out.append("renorm_scale_max")

        renorm_scale_vars = ["pdf0", "kappaFO0.5-kappaf2.", "kappaFO2.-kappaf0.5"]

        renorm_scale_var_hist = htotal[{"vars" : renorm_scale_vars}]

        renorm_scale_min = htotal_nominal.copy()
        renorm_scale_max = htotal_nominal.copy()

        renorm_scale_min.values()[...] = np.min(renorm_scale_var_hist.values(), axis=-1)
        renorm_scale_max.values()[...] = np.max(renorm_scale_var_hist.values(), axis=-1)

        additional_var_hists["renorm_scale_min"] = renorm_scale_min
        additional_var_hists["renorm_scale_max"] = renorm_scale_max

        # envelope of renormalization and factorization scale variations

        vars_out.append("renorm_fact_scale_min")
        vars_out.append("renorm_fact_scale_max")
        ...
        renorm_fact_scale_var_hist = htotal[{"vars" : renorm_fact_scale_vars}]

        renorm_fact_scale_min = htotal_nominal.copy()
        renorm_fact_scale_max = htotal_nominal.copy()

        renorm_fact_scale_min.values()[...] = np.min(renorm_fact_scale_var_hist.values(), axis=-1)
        renorm_fact_scale_max.values()[...] = np.max(renorm_fact_scale_var_hist.values(), axis=-1)

        additional_var_hists["renorm_fact_scale_min"] = renorm_fact_scale_min
        additional_var_hists["renorm_fact_scale_max"] = renorm_fact_scale_max
        ...
        qt20idx = htotal.axes["qT"].index(20.)

        vars_out.append("renorm_scale_pt20_min")
        vars_out.append("renorm_scale_pt20_max")

        renorm_scale_pt20_min = renorm_scale_min.copy()
        renorm_scale_pt20_max = renorm_scale_max.copy()

        renorm_scale_pt20_min[{"qT" : s[qt20idx:]}] = htotal_nominal[{"qT" : s[qt20idx:]}].view()
        renorm_scale_pt20_max[{"qT" : s[qt20idx:]}] = htotal_nominal[{"qT" : s[qt20idx:]}].view()

        additional_var_hists["renorm_scale_pt20_min"] = renorm_scale_pt20_min
        additional_var_hists["renorm_scale_pt20_max"] = renorm_scale_pt20_max

        vars_out.append("renorm_fact_scale_pt20_min")
        vars_out.append("renorm_fact_scale_pt20_max")

        renorm_fact_scale_pt20_min = renorm_fact_scale_min.copy()
        renorm_fact_scale_pt20_max = renorm_fact_scale_max.copy()

        renorm_fact_scale_pt20_min[{"qT" : s[qt20idx:]}] = htotal_nominal[{"qT" : s[qt20idx:]}].view()
        renorm_fact_scale_pt20_max[{"qT" : s[qt20idx:]}] = htotal_nominal[{"qT" : s[qt20idx:]}].view()

        additional_var_hists["renorm_fact_scale_pt20_min"] = renorm_fact_scale_pt20_min
        additional_var_hists["renorm_fact_scale_pt20_max"] = renorm_fact_scale_pt20_max

        if "muBup" in vars_unique:
            # envelope of all scale variations

            vars_out.append("renorm_fact_resum_scale_min")
            vars_out.append("renorm_fact_resum_scale_max")

            renorm_fact_resum_scale_vars = renorm_fact_scale_vars + resum_scale_vars

            renorm_fact_resum_scale_var_hist = htotal[{"vars" : renorm_fact_resum_scale_vars}]

            renorm_fact_resum_scale_min = htotal_nominal.copy()
            renorm_fact_resum_scale_max = htotal_nominal.copy()

            renorm_fact_resum_scale_min.values()[...] = np.min(renorm_fact_resum_scale_var_hist.values(), axis=-1)
            renorm_fact_resum_scale_max.values()[...] = np.max(renorm_fact_resum_scale_var_hist.values(), axis=-1)

            additional_var_hists["renorm_fact_resum_scale_min"] = renorm_fact_resum_scale_min
            additional_var_hists["renorm_fact_resum_scale_max"] = renorm_fact_resum_scale_max

            # envelope of all scale variations together with transition variations

            vars_out.append("renorm_fact_resum_transition_scale_min")
            vars_out.append("renorm_fact_resum_transition_scale_max")

            transition_vars = [var for var in vars_unique if "transition" in var]
            renorm_fact_resum_transition_scale_vars = renorm_fact_resum_scale_vars + transition_vars

            renorm_fact_resum_transition_scale_var_hist = htotal[{"vars" : renorm_fact_resum_transition_scale_vars}]

            renorm_fact_resum_transition_scale_min = htotal_nominal.copy()
            renorm_fact_resum_transition_scale_max = htotal_nominal.copy()

            renorm_fact_resum_transition_scale_min.values()[...] = np.min(renorm_fact_resum_transition_scale_var_hist.values(), axis=-1)
            renorm_fact_resum_transition_scale_max.values()[...] = np.max(renorm_fact_resum_transition_scale_var_hist.values(), axis=-1)

            additional_var_hists["renorm_fact_resum_transition_scale_min"] = renorm_fact_resum_transition_scale_min
            additional_var_hists["renorm_fact_resum_transition_scale_max"] = renorm_fact_resum_transition_scale_max

    if "muBup" in vars_unique:
        logger.info("adding envelopes for resummation scale variation")

        # envelope of resummation scales only

        vars_out.append("resum_scale_min")
        vars_out.append("resum_scale_max")

        resum_scale_var_hist = htotal[{"vars" : resum_scale_vars}]

        resum_scale_min = htotal_nominal.copy()
        resum_scale_max = htotal_nominal.copy()

        resum_scale_min.values()[...] = np.min(resum_scale_var_hist.values(), axis=-1)
        resum_scale_max.values()[...] = np.max(resum_scale_var_hist.values(), axis=-1)

        additional_var_hists["resum_scale_min"] = resum_scale_min
        additional_var_hists["resum_scale_max"] = resum_scale_max



    vars_out_axis = hist.axis.StrCategory(vars_out, name="vars")
    htotal_tmp = hist.Hist(*htotal.axes[:-1], vars_out_axis, storage = htotal._storage_type())

    for i, var in enumerate(vars_out_axis):
        if var in htotal.axes["vars"]:
            htotal_tmp[{"vars" : i}] = htotal[{"vars" : var}].view(flow=True)
        else:
            htotal_tmp[{"vars" : i}] = additional_var_hists[var].view(flow=True)
